...
void CompactDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		case DataParameter_DB_LEVELDB:
			...
			value = cursor_->value();
			mat = cvMat(1, 1000 * 1000 * 3, CV_8UC3, const_cast<char *>(value.data()) + sizeof(int));

			//datum.ParseFromString(iter_->value().ToString());
			break;
		case DataParameter_DB_LMDB:
			mat = cvMat(1, 1000 * 1000 * 3, CV_8UC3, (char *)(mdb_value_.mv_data) + sizeof(int));
...
void CompactDataLayer<Dtype>::InternalThreadEntry() {
	...
	for (int item_id = 0; item_id < batch_size; ++item_id) {
		...
		case DataParameter_DB_LEVELDB:
			...
			mat = cvMat(1, 1000 * 1000, CV_8UC3, const_cast<char *>(value.data()) + sizeof(int));
